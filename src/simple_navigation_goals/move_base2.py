    ...
    def scan_callback(self, data):
        ...
        if self.has_box:
            ...
            (left_outer,
             left_inner,
             right_inner,
             right_outer) =  [cloud[(cloud_angles > ang_from) & (cloud_angles < ang_to)]
                              for ang_from, ang_to in [(self.left_angle, self.left_angle + 10),
                                                       (self.left_angle - 10, self.left_angle),
                                                       (self.right_angle, self.right_angle + 10),
                                                       (self.right_angle - 10, self.right_angle)]]

            for to_mark, color in zip([left_outer, left_inner, right_inner, right_outer],
                                      ["red", "yellow", "pink", "orange"]):
                plt.plot(to_mark[:,0], to_mark[:,1],'+' , color=color)


            (left_outer_med,
             left_inner_med,
             right_inner_med,
             right_outer_med) = [op(x[:,-1]) for x, op in
                                 ((left_outer, np.max),
                                  (left_inner, np.min),
                                  (right_inner, np.min),
                                  (right_outer, np.max))]
            ...
            self.print_dists(left_outer_med, left_inner_med, right_inner_med, right_outer_med)
            cases = [
            ...
            for outer_med, inner_med, to_rotate in cases:
